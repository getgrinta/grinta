...
import { APP_MODE, COMMAND_HANDLER, ExecutableCommandSchema, type SearchEngine, SEARCH_ENGINE, COMMAND_PRIORITY } from "@getgrinta/core";
import { createPlugin, type PluginContext } from "@getgrinta/plugin";
import { buildCreateNoteCommand } from "./lib/notes";

export async function getSearchUrl({ query, currentSearchEngine }: {
    query: string,
    ...
    return match(currentSearchEngine)
        ...
        .with(
            SEARCH_ENGINE.SCIRA,
            () => `https://scira.com/search?q=${query}`,
        )
        .exhaustive();
}
...
export const PluginWebSearch = createPlugin({
    ...
    async addSearchResults(query, context) {
        const createNoteCommand = query.length > 0 ? [buildCreateNoteCommand(query, context)] : [];
        ...
        const completionList = await Promise.all(completions[1].map(async (completion: string) => {
        ...
        const completionsFiltered = completionList.filter((completion) => completion.label !== query);
