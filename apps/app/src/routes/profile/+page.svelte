    ...
    <div class="modal-action">
      <form method="dialog">
        <button type="button" onclick={stopSubscriptionCheck} class="btn"
          >{$_("profile.subscriptionModal.cancel")}</button
        >
      </form>
    </div>
  </div>
...
<div class="flex flex-1 flex-col">
  <TopBar {goBack}>
    <h1 slot="input" class="text-lg font-semibold w-full">
      {$_("profile.title")}
    </h1>
    <button type="button" slot="addon" class="btn btn-sm" onclick={signOut}
